    ...
    if (test === 1) {
        ...
        $.ajax({
            ...
            success: function (studentsJson, textStatus, xhr) {
                ...
                // take and check students attended in this course before
                $.ajax({
                    method: "GET",
                    url: "http://localhost:8080/api/users/findStudentsByCourse/" + courseId,
                    success: function (attendedStudentsJson, textStatus, xhr) {

                        studentsJson.forEach(function (itemStudent, indexStudent) {

                            for (var attendedStudent of attendedStudentsJson) {

                                if (Number(attendedStudent.id) === Number(itemStudent.id)) {
                                    $("#table").bootstrapTable("check", indexStudent);
                                    break;
                                }
                            } // END of inner for
